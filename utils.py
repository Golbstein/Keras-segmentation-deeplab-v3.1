...
def foreground_accuracy(y_true, y_pred):
    ...
    return K.sum(tf.to_float(~legal_labels & K.equal(true_pixels, pred_pixels))) / (0.001 + K.sum(tf.to_float(~legal_labels)))
...
class SegModel:
    ...
    def build_callbacks(self, tf_board = False, plot_process = True, steps = 50, plot_test_images = False):
        ...
        #cl = CyclicLR(base_lr=1e-6, max_lr=7e-3,
        ...
        stop_train = EarlyStopping(monitor = 'val_Mean_IOU', patience=100, verbose=1, mode = 'max')
        
        reduce_lr = ReduceLROnPlateau(monitor = 'val_Mean_IOU', factor=0.8,
    ...
    def create_seg_model(self, opt, net, load_weights = False, multi_gpu = True, to_compile = True, alpha = 1.):
        ...
        if self.coords:
            self.net = net + '_coords_' + str(self.sz[0])
    ...
    def create_generators(self, crop_shape = False, mode = 'train', resize_shape = (512,512), do_ahisteq = True,
                          n_classes = 21, horizontal_flip = True, vertical_flip = False, icnet = True, blur = False,
                          brightness=0.1, rotation=5.0, zoom=0.1, validation_split = .2, seed = 7):
                
        generator = MapillaryGenerator(folder = self.mainpath, mode = mode, n_classes = n_classes, do_ahisteq = do_ahisteq,
...
class MapillaryGenerator(Sequence):
    ...
    def __init__(self, folder='/workspace/datasets/', mode='train', n_classes=21, batch_size=1, resize_shape=None,
                 validation_split = .1, seed = 7, crop_shape=(640, 320), horizontal_flip=True, blur = False,
                 vertical_flip=False, brightness=0.1, rotation=5.0, zoom=0.1, icnet = True, do_ahisteq = True):
